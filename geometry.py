...
def canvas_dims() -> tuple[int, int, float]:
    ...
    Uses state.g_dl_w/h via attribute access so it always sees the
    latest values set by the render loop.
    """
...
def on_canvas() -> bool:
    ...
    # Reject clicks until the render loop has successfully measured and placed the
    # canvas at least once. Before that, g_dl_ox/oy are (0,0) and would falsely
    # match the layer panel area.
